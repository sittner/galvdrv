...
static esp_err_t bitstream_post_handler(httpd_req_t *req)
{
    ...
    if (req->content_len > CONFIG_LOADER_MAX_UPLOAD_BYTES) {
        ...
        return httpd_resp_sendstr(req, "{\"ok\":false,\"error\":\"bitstream exceeds max upload size\"}");
    }
    ...
    while (remaining > 0) {
        ...
        if (err != ESP_OK) {
            raw_bitstream_player_abort();
            ESP_LOGE(TAG, "Raw bitstream feed failed: %s", jtag_player_last_error());
            httpd_resp_set_status(req, "400 Bad Request");
            char response[256];
            snprintf(response, sizeof(response),
                     "{\"ok\":false,\"error\":\"%s\"}",
                     jtag_player_last_error());
    ...
    if (err != ESP_OK) {
        raw_bitstream_player_abort();
        ESP_LOGE(TAG, "Raw bitstream programming failed: %s", jtag_player_last_error());
        httpd_resp_set_status(req, "400 Bad Request");
        char response[256];
        snprintf(response, sizeof(response),
                 "{\"ok\":false,\"error\":\"%s\"}",
